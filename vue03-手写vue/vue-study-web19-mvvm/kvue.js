...
class Compiler {
  ...
  compileElement(node) {
    ...
    Array.from(attrs).forEach(attr => {
      ...
      // 事件处理
      // 冬瓜冬瓜我是西瓜
      if (attrName.indexOf('k-on') === 0 || attrName.indexOf('@') === 0) {
        const ev = attrName.indexOf('k-on') === 0 ?
          attrName.substring(5) :
          attrName.substring(1);
        const fn = this.$vm.$options.methods[exp]
        fn && node.addEventListener(ev, fn.bind(this.$vm, node))
      } else if (attrName.indexOf('k-') === 0) {
        // 截取指令名称 text
